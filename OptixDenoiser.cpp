...
OPTIXDENOISER_API OptixResult createDenoiser(OptixDeviceContext context, OptixDenoiserModelKind kind, const OptixDenoiserOptions* options, OptixDenoiser* denoiser)
{
	return optixDenoiserCreate(context, kind, options, denoiser);
}
...
OPTIXDENOISER_API OptixResult invokeDenoiser(
	...
	const  OptixDenoiserGuideLayer* guideLayer, const OptixDenoiserLayer* inputLayers, unsigned int numInputLayers, unsigned int inputOffsetX, unsigned int inputOffsetY, const OptixImage2D* outputLayer,
	CUdeviceptr scratch, size_t scratchSizeInBytes)
{
	return optixDenoiserInvoke(denoiser, stream, params, denoiserState, denoiserStateSizeInBytes, guideLayer, inputLayers, numInputLayers, inputOffsetX, inputOffsetY,
